...
import TableAction from "@/components/table-action-admission/table-action-admission";
import {
  Dialog,
  DialogContent,
  DialogTrigger,
...
import { IConsultation } from "@/constants/interface";
...
const forms: IConsultation[] = [
  ...
  },
  {
    ...
    phoneNumber: "0912345678",
    status: "Đang chờ",
    dateApplied: "04-10-2024",
  },
];

const formsPerPage = 10;
const fields: {
  name: keyof IConsultation;
  label: string;
  placeholder: string;
  type: string;
}[] = [
...
const ManageConsultingRegistration = () => {
  ...
  const handleSearch = () => {
    const results = forms.filter((form) => {
      const matchesStudent = form.name
        .toLowerCase()
      ...
      <div className="flex justify-between">
        ...
        <Dialog open={isOpen} onOpenChange={setIsOpen}>
          ...
          <DialogContent className="sm:max-w-[700px] ">
            <FormPopUp label="tư vấn" fields={fields} onSubmit={handleSubmit} />
          </DialogContent>
        </Dialog>
      </div>
        ...
        <Table>
          <TableCaption>
            <div className="flex items-center gap-3">
              <span className="font-bold">Danh sách đăng ký tư vấn</span>
              <span>(Hiện có {filteredForms.length} người tư vấn)</span>
            </div>
            <Dialog open={isFilterOpen} onOpenChange={setIsFilterOpen}>
              ...
              <DialogContent className="sm:max-w-[700px]">
                <DialogHeader>
                  <DialogTitle>Lọc danh sách tư vấn</DialogTitle>
                </DialogHeader>
                <div className="flex flex-col gap-5 my-4">
                  ...
                  <div className="flex items-center w-full">
                    <Label className="w-1/4">Trạng thái tư vấn:</Label>
                    <Select onValueChange={setStatusFilter}>
                      <SelectTrigger className="w-3/4">
                  ...
                  {fields.map((field) => (
                    <TableCell key={field.name} className="min-w-[200px]">
                      {field.name === "status" ? (
                        ...
                        //         {/* Thêm thông tin khác của sinh viên nếu cần */}
                        //       </div>
                        //     ))
                        //   )
                        form[field.name] // TypeScript sẽ hiểu `field.name` là một key hợp lệ của FormType
                      )}
                    </TableCell>
                  ))}
...
export default ManageConsultingRegistration;
